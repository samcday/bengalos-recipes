...
sed -i "s/@@SUITE@@/${SUITE}/" /etc/apt/sources.list.d/phosh-nightly.sources

# Setup repo priorities so phosh-nightly comes first
cat > /etc/apt/preferences.d/00-phosh-priority << EOF
Package: *
Pin: release o=Phosh
Pin-Priority: 700
EOF
